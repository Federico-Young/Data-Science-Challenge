...
def plot_bivariant(final_iv, feature, feature_name_in_plot, title_target_in_plot, specific_x_label_rotation = -1, categories_order_by_STR = True, string_int_ordering = False):
    ...
    leg = plt.legend()
    ax2 = plot_info['EVENT_RATE'].plot( secondary_y=True, style='o-', color = 'orange', grid = True,  label=title_target_in_plot)
    leg2 = plt.legend()
    ax.set_xlabel(feature_name_in_plot, fontsize=15)
    ax.set_ylabel('volumen', fontsize=15, labelpad=20)
    ax.right_ax.set_ylabel(title_target_in_plot, rotation = 270, fontsize=15, labelpad=30)
    ax.set_title(title_target_in_plot +" para " + feature_name_in_plot + " feature "  , fontsize=15)
    ...
    if amount_of_bins > max_amount_of_bins_for_visible_labels:
        ax.set_xticklabels(plot_info['RANGE'] ,fontsize = 15, rotation=90)
